...
其基本思想是：先设定一个学习率 $\eta$，参数沿梯度的反方向移动。假设需要更新的参数为$w$，梯度为$g$，则其更新策略可表示为：
$$w \leftarrow w - \eta * g$$
...
我们以 目标函数（损失函数） $f(x)= x^2$  为例来看一看梯度下降是如何工作的。
...
这里x为模型参数，使用x = 10 作为初始值，并设 学习率$eta = 0.2$，使用梯度下降法 对x 迭代10次
...
$$累计梯度更新：v \leftarrow \alpha v + (1 - \alpha)g$$$$梯度更新：x \leftarrow x - \eta*v$$$$\alpha 为动量参数，v累计梯度，\eta 为学习率$$


为了更好的观察动量带来的好处，我们使用一个新函数 $f(x)=0.1x_1^2+2x_2^2$ ，这里$x_1$和$x_2$的系数分别是 0.1 和 2， 这就使得$x_1$和$x_2$ 的梯度值相差一个量级，如果使用相同的学习率，$x_2$ 的更新幅度会较$x_1$ 的更大些。
...
从结果来看，和我们预料的一样：使用相同的学习率，$x_2$ 的更新幅度会较$x_1$ 的更大些，变化快得多。

接下来，我们依然使用传统梯度下降算法，<mark>将学习率设置为 0.6</mark>。

因为$x_2$的更新路径如下，这会导致 $x_2$ 的值越来越发散：

 $$x_2 = x_2 - \eta * gd_2 = x_2 - 0.6 *4 x_2 = -1.4x_2$$ 
...
- 如果选择较小的准确率。可以确保不会朝$x_2$方向发生偏离，但在$x_1$方向收敛会缓慢。
- 如果选择较大的准确率，$x_1$方向会收敛很快，但在$x_2$方向就不会向最优点靠近。下面将
...
<mark>$累计梯度更新：v \leftarrow \alpha v + (1 - \alpha)g$</mark>

<mark>$权重更新： x \leftarrow x - \eta*v$</mark>

<mark>$\alpha$ 为动量参数，$v$是累计梯度，$\eta$ 为学习率</mark>
...
## 3、Adagrad
Adagrad优化算法 被称为==自适应学习率优化算法==，之前我们讲的随机梯度下降对所有的参数都使用的固定的学习率进行参数更新，但是不同的参数梯度可能不一样，所以需要不同的学习率才能比较好的进行训练。

> 举例：
> \
> 假设损失函数是 $Loss=x_1^2+10x_2^2$，即我们的目标是学习$x_1$和$x_2$的值，让Loss尽可能小。
> 显然当$x_1=0, x_2=0$ 时，Loss取得最小值。
> \
> 假设x和y的初值分别为 $x_1=40, x_2=20$
> 此时对Loss函数进行求导，$x_1$和$x_2$梯度分别是 $\frac{\partial loss}{\partial x_1}=80, \frac{\partial loss}{\partial x_2}=400$
> 显然$x_1$将要移动的距离小于$x_2$将要移动的距离
> 但是实际上$x_1$离最优值 $loss=0$ 更远，差距是40，$x_2$离最优值 $loss=0$ 近一些，距离是20。
> \
> 因此SGD给出的结果并不理想。
...
权重更新
$$x \leftarrow x - \frac{\eta}{\sqrt {r + \delta}}*g$$
...
权重更新：$x \leftarrow x - \frac{\eta}{\sqrt {r + \delta}}*g$
...
1、梯度方面增加了momentum，使用累积梯度： $m \leftarrow \beta_1 m+(1- \beta_1)g$

2、同 RMSProp 优化算法一样, 对学习率进行优化：$v \leftarrow \beta_2 v + (1 - \beta_2)g^2$

3、偏差纠正：$\hat{m}_t = \frac{m}{1 -  \beta_1^t }$，$\hat{v} = \frac{v}{1 - \beta_2^t}$



再如上3点改进的基础上，权重更新：$x \leftarrow x - \frac{\eta}{\sqrt {\hat{m} + \delta}}*\hat{v}$


> **为啥要偏差纠正**
>
> $t$ 为更新次数，第$t$次更新时，$m_t \leftarrow \beta_1 m_{t-1}+(1- \beta_1)g$
> 当$t=1$时，$m_1=\beta_1*m_0+(1-\beta_1)*g_1$，由于$m_0$的初始是0，且β接近1，因此$t$较小时，$m$的值是偏向于0的
...
> 1、Gradient Descent
> 权重更新： $\theta \leftarrow \theta - \eta*g$

> 2、Gradient Descent with momentum
> 累计梯度更新：$v \leftarrow \alpha v + (1 - \alpha)g$
> 权重更新： $\theta \leftarrow \theta - \eta*v$
> $\alpha$ 为动量参数，$v$累计梯度，$\eta$为学习率

> 3、Adagrad
> 累计平方梯度：$r \leftarrow r + g^2$
> 权重更新：$\theta \leftarrow \theta - \frac{\eta}{\sqrt {r + \delta}}*g$
...
> 权重更新：$\theta \leftarrow \theta - \frac{\eta}{\sqrt {\hat{r} + \delta}}*\hat{v}$
> 其中：
